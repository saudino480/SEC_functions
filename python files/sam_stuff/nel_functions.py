    ...
    for k in dictionary.keys():
        ...
        for index in range(len(target_list)):
            ...
            if isinstance(df, pd.DataFrame):
                ...
                if (np.array([re.findall('^vol_\d+', df_col[x]) == [] for x in range(len(df_col))]).all() \
                              & np.array([re.findall('^price_\d+', df_col[x]) == [] for x in range(len(df_col))]).all() \
                              & np.array([re.findall('^instrument_type', df_col[x]) == [] for x in range(len(df_col))]).all() \
                              & np.array([re.findall('^product_type', df_col[x]) == [] for x in range(len(df_col))]).all() \
                              & np.array([re.findall('additional', df_col[x]) == [] for x in range(len(df_col))]).all()):
                ...
                for x in range(df.shape[0]):
                    ...
                    for y in range(df.shape[1]):
                        ...
                        if df.iloc[x, y] != '':
                            ...
                            df.iloc[x, y] = re.sub(r'\(\d{1}\)', '', df.iloc[x, y])
