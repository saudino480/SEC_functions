...
def volumeCleaner(dataFrame, key_words = "", stop_words = "", verbose = False, key_override = False):
    ...
    for colName in dataFrame.columns:
        ...
        if volume_bool:
            ...
            if key_override:
                new_vals = np.array([volume_key]*length)
            else:
...
def dateCleaner(dataFrame, key_words = "", stop_words = "", impute = False, verbose = False, \
                key_override = False, key_impute = "placeholder", new_override = False, value_ids = ["placeholder"],\
                ad_column = "additional"):
    dataFrame = dataFrame.rename(str.lower, axis = 'columns')
    #dataFrame = dataFrame.loc[:,~dataFrame.columns.duplicated()]
    if key_words == "":
        ...
        for col in unprocessed_cols:
            for value_id in value_ids:
                if any(dataFrame[col].str.contains(value_id)):
                    ...
                    dataFrame[col] = dataFrame[col].str.replace("[^" + value_id + "]", "")
